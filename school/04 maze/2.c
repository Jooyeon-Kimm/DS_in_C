...
int** mark;
int sizerow, sizecol;
int enterrow, entercol, exitrow, exitcol;
FILE* f2;
...
int main() {
	...
	if ((f = fopen("in.txt", "r")) == NULL)		// 파일 열기
	{
		printf("파일이 열리지 않습니다.\n");
		exit(1);
	}
	
	if ((f2 = fopen("out.txt", "w")) == NULL)		// 파일 열기
	{
		printf("파일이 열리지 않습니다.\n");
		exit(1);
	}
	...
	// 입구, 출구 읽어오기
	scanf("%d %d %d %d", &enterrow, &entercol, &exitrow, &exitcol);
...
void path()
{
	...
	element position;
	mark[1][1] = 1; top = 0;
	stack[0].row = enterrow; stack[0].col = entercol; stack[0].dir = 0;
	...
	if (found) {
		for (int i = 0; i <= top; i++) 
			maze[stack[i].row][stack[i].col] = 120;				// 'x' 아스키 코드 값
		maze[row][col] = 120;
		maze[exitrow][exitcol] = 120;
